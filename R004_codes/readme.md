...
# Complete RNA004 Workflow

For a typical inference-only analysis, the complete workflow is:
...
For users who want to train or fine-tune NanoNmD:
...
For users who only want to perform Nm prediction using the pre-trained RNA004 model:
...
### 2. Prepare the RNA004 Dorado model
...
and configure the Dorado/model paths in the preprocessing script.

### 3. Place raw `.pod5` files
...
### 4. Generate NanoNmD features

```bash
sbatch 00_preprocess_raw_data.sh
```

This generates:

```text
05_tsv_out/feature.R004.txt
```
...
The prediction results will be generated in:
...
The NanoNmD RNA004 workflow can be summarized as:
...
For most users, only **Steps 1 and 2** are required:
...
**Step 3 is optional** and is only required when training or fine-tuning NanoNmD with labelled data.
