...
def read_motifs_from_file(file_name):
    """
    Reads a list of motifs from a file, one motif per line.
    
    Args:
        file_name (str): Path to the file containing motifs.

    Returns:
        list: A list of motifs read from the file.
    """
...
###def get_ref_info(args):
###    params, chrom = args
###    motif_seq = read_motifs_from_file(params['motif'])  # List of 5mers
###    ref_fasta = pysam.FastaFile(params['ref'])
###    seq = ref_fasta.fetch(chrom).upper()
###    seq_array = get_ref_to_num(seq)
###
###    fwd_pos_array, rev_pos_array = set(), set()  # Use sets to avoid duplicates
###
###    for motif in motif_seq:
###        # Find positions for each motif in forward and reverse strands
###        fwd_motif_anchor = np.array([m.start(0) for m in re.finditer(r'(?={})'.format(motif), seq)])
###        rev_motif_anchor = np.array([m.start(0) for m in re.finditer(r'(?={})'.format(revcomp(motif)), seq)])
###        
###        # Add positions to the respective sets
###        fwd_pos_array.update(fwd_motif_anchor)
###        rev_pos_array.update(rev_motif_anchor)
###
###    # Convert sets to sorted arrays
###    fwd_pos_array = np.array(sorted(fwd_pos_array)).astype(int) if fwd_pos_array else None
###    rev_pos_array = np.array(sorted(rev_pos_array)).astype(int) if rev_pos_array else None
###	
###    return chrom, seq_array, fwd_pos_array, rev_pos_array
...
def get_ref_info(args):
    ...
    # Skip motif processing if params['motif'] is None
    motif_seq = None
    if params['motif']:
        motif_seq = read_motifs_from_file(params['motif'])  # List of 5mers
    ...
    if motif_seq:  # Process motifs only if provided
        for motif in motif_seq:
            # Find positions for each motif in forward and reverse strands
            fwd_motif_anchor = np.array([m.start(0) for m in re.finditer(r'(?={})'.format(motif), seq)])
            rev_motif_anchor = np.array([m.start(0) for m in re.finditer(r'(?={})'.format(revcomp(motif)), seq)])
...
def get_events(signal, move, norm_type):
    ...
    for i in range(len(move_index)-1):
        ...
        data[i, 3]=(sig_len)
        data[i, 2]=np.median(signal[prev:sig_end])
        data[i, 0]=np.mean(signal[prev:sig_end])
        data[i, 1]=np.std(signal[prev:sig_end])
...
def write_to_npz(output_file_path, mat, base_qual, base_seq, ref_seq, real_ref_seq, label, ref_coordinates, read_name, ref_name, window, norm_type):
    np.savez(output_file_path, mat=mat, base_qual=base_qual, base_seq=base_seq, ref_seq=ref_seq,real_ref_seq=real_ref_seq, label=label, ref_coordinates=ref_coordinates, read_name=read_name, ref_name=ref_name, window=window, norm_type=norm_type)

def write_to_tsv(output_file_path, mat, base_qual, base_seq, ref_seq, real_ref_seq, label, ref_coordinates, read_name, ref_name):
    header = "Read_Name\tRef_Name\tRef_Coordinate\tBase_Sequence\tReference_Sequence\treal_ref_seq\tFeatures1\tFeatures2\tFeatures3\tFeatures4"
    with open(output_file_path, 'w') as f:
        f.write(header + '\n')
        for i in range(len(read_name)):
            read = read_name[i]
            ref = ref_name[i]
            coord = ref_coordinates[i]
            base_sequence = ''.join(num_to_base_map[num] for num in base_seq[i])
            reference_sequence = ''.join(num_to_base_map[num] for num in ref_seq[i])
            real_ref_sequence = ''.join(num_to_base_map[num] for num in real_ref_seq[i])
            base_quality = '|'.join(map(str, base_qual[i]))
#            features = '|'.join(map(str, mat[i].flatten()))
# Extract the four parts from the feature matrix
            mean_values = '|'.join(map(str, mat[i][:, 0]))  # Extract mean values
            median_values = '|'.join(map(str, mat[i][:, 2]))  # Extract median values
            std_values = '|'.join(map(str, mat[i][:, 1]))  # Extract standard deviation values
            length_values = '|'.join(map(str, mat[i][:, 3]))  # Extract length values

# Combine the four parts, tab-separated
            features = f"{mean_values}\t{median_values}\t{std_values}\t{length_values}"
            line = f"{read}\t{ref}\t{coord}\t{base_sequence}\t{reference_sequence}\t{real_ref_sequence}\t{features}"
            f.write(line + '\n')

                       
def get_output(params, output_Q, process_event):
    output=params['output']
    output_tsv=params.get('output_tsv', None)    
    ...
    output_tsv_path=os.path.join(output,'%s.features.%d.tsv' %(params['prefix'], chunk))
        
    mat, base_qual, base_seq, ref_seq, real_ref_seq, label=[], [], [], [], [], []
    ...
    while True:
            if process_event.is_set() and output_Q.empty():
                ...
                try:
                    ...
                    #per_site_features, per_site_base_qual, per_site_base_seq, per_site_ref_seq,per_site_motif, per_site_ref_coordinates, per_site_label, read_name_array, ref_name_array
                    
                    mat.append(res[0]) # per_site_features
                    print(f"Appended to mat: {res[0].shape}")
                    # print(f'{res[0][0:5]}')
                    base_qual.append(res[1]) # per_site_base_qual
                    base_seq.append(res[2]) # per_site_base_seq
                    ref_seq.append(res[3]) # per_site_ref_seq
                    real_ref_seq.append(res[4]) # per_site_motif
                    print(f"Appended to real_ref_seq: {res[4].shape}")
                    # print(f'{res[4]}')
                    ref_coordinates.append(res[5]) # per_site_ref_coordinates
                    label.append(res[6]) # per_site_label
                    read_name.append(res[7]) # read_name_array
                    ref_name.append(res[8]) # ref_name_array
                    ...
                    if read_count%reads_per_chunk==0 and len(mat)>0:
                        ...
                        real_ref_seq=np.vstack(real_ref_seq).astype(np.int8)
                        ...
                        write_to_npz(output_file_path, mat, base_qual, base_seq, ref_seq, real_ref_seq, label, ref_coordinates, read_name, ref_name, window=params['window'], norm_type=params['norm_type'])
                        write_to_tsv(output_tsv_path, mat, base_qual, base_seq, ref_seq, real_ref_seq, label, ref_coordinates, read_name, ref_name)
    ...
    if read_count>0 and len(mat)>0:
        ...
        write_to_npz(output_file_path, mat, base_qual, base_seq, ref_seq, real_ref_seq, label, ref_coordinates, read_name, ref_name, params['window'], params['norm_type'])
        
        write_to_tsv(output_tsv, mat, base_qual, base_seq, ref_seq, real_ref_seq, label, ref_coordinates, read_name, ref_name)
...
def process(params, ref_pos_dict, signal_Q, output_Q, input_event, ref_seq_dict, labelled_pos_list):
    ...
    while True:
        ...
        try:
            ...
            mat=get_events(signal, move, norm_type) # features table
            
            if seq_type=='rna':
                mat=np.flip(mat,axis=0)

            # feautres of each site for each kmer  
            # print(f'Debug - Ref: {ref_name}, Read id: {read_dict['name']}')  
            # print(f'Debug -  {pos_list_candidates.shape}, {pos_list_candidates[:5]}')  
            ...
            ref_array = ref_seq_dict[ref_name][:, 0]
            # per_site_motif=np.asarray([ref_array[coord-window:coord+window+1] for coord in per_site_ref_coordinates], dtype='object')

            # per_site_motif=np.array([ref_array[coord-window:coord+window+1] for coord in per_site_ref_coordinates])
            per_site_motif = np.asarray([
                ref_array[coord - window: coord + window + 1]
                if 0 <= coord - window and coord + window + 1 <= len(ref_array)
                else np.full((2 * window + 1,), 4)  # or other padding character
                for coord in per_site_ref_coordinates
            ], dtype = 'object')
            ...
            # print(f"Debug - Ref Seq: {ref_seq}, Sit Seq Shape: {per_site_ref_seq}, Sample Ref Coords: {per_site_ref_coordinates[:5]}")
            # print(f"Debug - Motif {per_site_motif},Sample Ref Coords: {per_site_ref_coordinates[:5]}")
            read_chunks=[per_site_features, per_site_base_qual, per_site_base_seq, per_site_ref_seq, per_site_motif, per_site_ref_coordinates, per_site_label, read_name_array, ref_name_array]
...
def call_manager(params):        
    ...
    with mp.Pool(processes=params['threads']) as pool:
        ...
        for r in res:
            ...
            print(f"Sequence Array Length: {len(seq_array)}")
            print(f"Forward Positions: {fwd_pos_array}")
            print(f"Reverse Positions: {rev_pos_array}")
            ...
            if params['pos_list']:
                ...
                for strand in [0,1]:
                    if ref_pos_dict[chrom][strand] is None:
                        print(f"Strand {strand} of chromosome {chrom} is None. Skipping.")
                        continue
...
if __name__ == '__main__':
    ...
    parser.add_argument("--motif", help='Text file containing a list of motif for generating features', type=str)
    ...
    parser.add_argument("--output_tsv", help='Path to output TSV file.', type=str)
